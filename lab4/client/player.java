...
public class player  extends gobject {
  Color m_color=Color.green;
    ...
    public player(double x, double y, double r, Color mycolor, double alpha) throws Exception {
    ...
    public  void huntershots(glist game) {
    	double d=9999;
    	double d2=0;
    	gobject res=null;
    	for(gobject o2:game.m_clients) 
            if (o2!=this)
    ...
    public void update(glist game, double delta_t) {
      ...
      //  acceleration = new PVector(0,0);
        ...
        double brake_strength = (m_speed>0 ? 90:60);
        double acceleration = keycontrol[M_THROTTLE]*60 - 
    ...
    public void draw(Graphics2D g) {
        ...
        if(drawball)
          gg.fillRect((int)(m_r+m_x), (int)(m_y), (int)(m_r/2), 4);
...
/*        
        ...
        double x2=x1+100*Math.cos(m_alpha);
        double y2=y1+100*Math.sin(m_alpha);
        double x0=x1+110*Math.cos(m_alpha);
        double y0=y1+110*Math.sin(m_alpha);;
        double l=Math.abs( (y2-y1)*x0-(x2-x1)*y0+x2*y1-y2*x1 ) / Math.sqrt(Math.pow(y2-y1, 2)  + Math.pow(x2-x1, 2));
    	System.out.println("-- m_x= "+l+"  m_y= "+m_y+" d= "+l+"  ");


        g.setColor(Color.blue);
        g.drawLine((int)x1, (int)y1, (int) (x2), (int) (y2));
   //     g.drawLine((int)(x1-1), (int)(y1-1), (int) (Mx-1), (int) (My-1));
    	//System.out.println("-- m_x= "+m_x+"  m_y= "+m_y+" m_r= "+m_r+"  ");
         */
